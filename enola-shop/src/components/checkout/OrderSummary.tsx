...
export default function OrderSummary() {
  ...
  return (
    <div className="bg-white p-9 rounded-[10px] border border-brand-border h-fit sticky top-24 space-y-8">
      <h2 className="text-xl font-normal text-brand-ink uppercase tracking-tight">Захиалгын хураангуй</h2>
          ...
          <div key={item.id} className="flex gap-4 group">
            ...
            <div className="flex-1 flex flex-col justify-center gap-1">
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-[10px] text-brand-hint font-medium tracking-wide uppercase">{item.quantity} x {formatCurrency(item.price)}</span>
      ...
      <div className="space-y-4 pt-6 border-t border-brand-border">
        ...
        <div className="flex justify-between items-end pt-5 border-t border-brand-border">
          <span className="text-[12px] font-medium text-brand-sub uppercase tracking-[2px]">Нийт дүн</span>
          <span className="text-2xl font-semibold text-brand-ink">{formatCurrency(total)}</span>
        </div>
      </div>

      <div className="text-[10px] text-brand-hint leading-relaxed text-center px-4 uppercase tracking-wider">
