  ...
  if (items.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center bg-brand-base">
         ...
         <h2 className="text-xl font-normal text-brand-ink mb-2 tracking-[1px] uppercase">Your bag is empty</h2>
         <p className="text-brand-sub font-normal mb-8 text-[13px]">Shop our latest collection to define your style.</p>
         <Link to="/shop" className="text-brand-ink text-[12px] font-medium tracking-wider uppercase border-b border-brand-ink pb-0.5">
           CONTINUE SHOPPING
         </Link>
      </div>
    )
  ...
  return (
    <div className="bg-brand-base min-h-screen py-20 pb-40">
      <div className="max-w-7xl mx-auto px-4">
        <h1 className="text-3xl font-normal text-brand-ink tracking-[2px] mb-12 uppercase">YOUR SHOPPING BAG</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
               ...
               <div key={item.id} className="bg-white p-6 rounded-[10px] border border-brand-border flex flex-col sm:flex-row gap-8">
                 ...
                 <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start gap-4 mb-4">
                       <div>
                          <Link to={`/shop?category=${item.categorySlug}`} className="text-[9px] uppercase tracking-[1.5px] font-bold text-brand-hint hover:text-brand-ink transition-colors">
                            {item.category}
                          </Link>
           ...
           <div className="w-full lg:w-[400px] flex-shrink-0">
              <div className="bg-white p-9 rounded-[10px] border border-brand-border shadow-none sticky top-24">
                 <h3 className="text-xl font-normal text-brand-ink mb-10 tracking-wide uppercase">Bag Summary</h3>
                 
                 <div className="space-y-6 mb-10">
                    <div className="flex justify-between items-center text-[12px] font-normal text-brand-sub uppercase tracking-wider">
                       <span>SUBTOTAL</span>
                       <span className="text-brand-ink font-medium">{formatCurrency(totalPrice())}</span>
                    </div>
                    <div className="flex justify-between items-center text-[12px] font-normal text-brand-sub uppercase tracking-wider border-b border-brand-border pb-6">
                       <span>ESTIMATED DELIVERY</span>
                       <span className="text-brand-success font-bold">FREE</span>
                    </div>
                    <div className="flex justify-between items-end pt-5">
                       <span className="text-[13px] font-medium text-brand-sub tracking-[1px] uppercase">TOTAL AMOUNT</span>
                 ...
                 <Link 
                   to="/checkout"
                   className="w-full h-12 bg-brand-ink text-brand-base rounded-[6px] flex items-center justify-center gap-3 font-normal text-[12px] uppercase tracking-[1.5px] hover:bg-brand-ink2 transition-all"
                 >
                 ...
                 <div className="mt-10 pt-10 border-t border-brand-border flex items-center justify-center gap-6 opacity-40 grayscale">
                    <span className="text-[10px] font-medium tracking-[2px] text-brand-ink uppercase">QPAY</span>
                    <span className="text-[10px] font-medium tracking-[2px] text-brand-ink uppercase">SOCIALPAY</span>
                    <span className="text-[10px] font-medium tracking-[2px] text-brand-ink uppercase">BANK</span>
                 </div>
              </div>
           </div>
        </div>
