    ...
    <div className="flex min-h-screen bg-brand-base">
      ...
      <aside className="w-64 bg-brand-surface border-r border-brand-border flex flex-col">
        <div className="p-8 border-b border-brand-border">
          <span className="text-brand-ink font-normal text-xl uppercase tracking-[4px]">
            ANTIGRAVITY
          </span>
          <p className="text-[10px] text-brand-hint mt-1 uppercase tracking-[2px]">Admin Console</p>
        </div>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="flex justify-between items-center mb-10">
          <h1 className="text-2xl font-normal text-brand-ink uppercase tracking-tight">System Overview</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          {stats.map((stat, i) => (
            <div key={i} className="bg-white border border-brand-border rounded-[8px] p-6 relative group">
              <p className="text-brand-hint text-[10px] font-bold uppercase tracking-wider mb-2">{stat.label}</p>
        ...
        <div className="bg-white border border-brand-border rounded-[10px] overflow-hidden">
          <div className="p-6 border-b border-brand-border flex justify-between items-center">
            <h2 className="text-[13px] font-medium text-brand-ink uppercase tracking-wider">Recent Transactions</h2>
            <button className="text-[11px] text-brand-ink font-bold uppercase hover:underline flex items-center gap-1">View All <ArrowUpRight className="w-3 h-3" /></button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-brand-surface text-brand-hint text-[10px] font-bold uppercase tracking-[1.5px]">
                <tr>
                ...
                {orders.map((order, i) => (
                  <tr key={i} className="border-b border-brand-border hover:bg-brand-surface transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <span className={`px-2.5 py-1 rounded-[4px] text-[10px] font-bold uppercase tracking-wider ${statusStyles[order.status as keyof typeof statusStyles]}`}>
                        {order.status}
                      </span>
                    </td>
                  </tr>
